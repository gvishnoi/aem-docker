...
ENV AEM_HOME=/opt/aem \
    ...
    RUN_MODE=author \
    PORT=5502 \
    JVM_OPTS="-server -Xms4g -Xmx4g -XX:+UseG1GC -Djava.awt.headless=true" \
...
# Reuse same entrypoint logic
COPY --from=eclipse-temurin:17-jdk /etc/os-release /etc/os-release
RUN printf '%s\n' \
    '#!/usr/bin/env bash' \
    'set -euo pipefail' \
    'echo "[AEM] Home: ${AEM_HOME}"' \
    ...
    'else' \
    '  echo "[AEM] Existing repo. Starting..."' \
